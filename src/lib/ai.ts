...
// Primary AI configuration with DeepSeek Chat V3 as requested
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 429 && retryCount < 2) {
        const waitTime = (retryCount + 1) * 10; // Increased wait time for DeepSeek
...
export function initializeAI(): void {
  ...
  console.log('✅ AI Service initialized with DeepSeek Chat V3');
