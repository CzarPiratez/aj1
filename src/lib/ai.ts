...
// Primary AI configuration with models that have better rate limits
...
const AI_CONFIGS = [
  {
    ...
    description: 'Primary AI engine - Llama 3.1 8B (Free with better limits)'
  },
  {
    type: 'openrouter' as const,
    name: 'Llama 3.1 8B Backup',
    key: apiKey2 || '',
    default_model: 'meta-llama/llama-3.1-8b-instruct:free',
    description: 'Backup AI engine - Llama 3.1 8B (Free with better limits)'
  },
  {
    type: 'openrouter' as const,
    ...
    description: 'Fallback AI engine - DeepSeek Chat V3 (Free)'
  },
  {
    type: 'openrouter' as const,
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 429 && retryCount < 2) {
        const waitTime = (retryCount + 1) * 5; // Increased wait time
