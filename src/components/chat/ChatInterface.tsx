...
export function ChatInterface({ onContentChange, profile }: ChatInterfaceProps) {
  ...
  const generateSimpleResponse = (userInput: string): string => {
    ...
    } else if (input.includes('post') && (input.includes('job') || input.includes('jd'))) {
      return "🚧 The job posting tool is currently under construction. We're working on rebuilding this feature from scratch to provide you with an even better experience. Please check back later!";
    } else {
  ...
  const handleToolAction = (toolId: string, message: string) => {
    // Special handling for Post Job tool - show construction message
    if (toolId === 'post-job-generate-jd' || toolId === 'match-candidates') {
      const constructionMessage: Message = {
        id: Date.now().toString(),
        content: "🚧 This feature is currently under construction. We're rebuilding it from scratch to provide you with an even better experience. Please check back later!",
        sender: 'assistant',
        timestamp: new Date(),
        type: 'suggestion'
      };
      
      setMessages(prev => [...prev, constructionMessage]);
      return;
    }
